...
| `master.py` | `category`, `product`, `product_option`, `pp_options`, `zone`, `res`, `equip`, `equip_load_spec`, `trans`, `trans_task_bat_threshold`, `ra_motion_step`, `ai_model` |
| `order.py` | `ord`, `ord_detail`, `ord_pp_map`, `pattern`, `ord_txn`, `ord_stat`, `ord_log` |
| `item.py` | `item` |
| `equipment.py` | `equip_task_txn`, `equip_stat`, `pp_task_txn`, `log_err_equip`, `log_data_equip` |
| `transport.py` | `trans_task_txn`, `trans_stat`, `log_err_trans`, `log_data_trans` |
| `inspection.py` | `insp_task_txn`, `ai_inference_txn`, `insp_stat` |
| `models_mgmt.py` | `chg_location_stat`, `strg_location_stat`, `ship_location_stat`, `alerts_stat`, 각종 log 테이블 |
...
| `03_create_customer_order.py` | Phase 1-1: 고객 주문 생성 (웹 UI 대체) |
